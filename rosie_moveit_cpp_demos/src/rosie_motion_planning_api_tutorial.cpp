/* Author: Ludovic Montagnon based on moveit2 tutorials main branch and Sachin Chitta, Michael Lautman script*/
...
static const rclcpp::Logger LOGGER = rclcpp::get_logger("rosie_motion_planning_api_tutorial");
...
int main(int argc, char** argv)
{
  ...
  std::shared_ptr<rclcpp::Node> motion_planning_api_tutorial_node =
      rclcpp::Node::make_shared("rosie_motion_planning_api_tutorial", node_options);
  ...
  //planning_scene->getCurrentStateNonConst().setToDefaultValues(joint_model_group, "front_home");
  ...
  moveit_visual_tools::MoveItVisualTools visual_tools(motion_planning_api_tutorial_node, "arm_world",
  ...
  pose.header.frame_id = "arm_world";
  ...
  moveit_msgs::msg::Constraints pose_goal =
      kinematic_constraints::constructGoalConstraints("leftend_effector_link", pose, tolerance_pose, tolerance_angle);

  // 1) Utiliser l’état courant comme start state côté MoveGroupInterface
  move_group.setStartStateToCurrentState();

  // 2) Récupérer l’état courant (attend jusqu’à 2 s si besoin)
  auto current_state = move_group.getCurrentState(2.0);
  if (!current_state) {
    RCLCPP_ERROR(LOGGER, "Etat courant non disponible (joint_states ?)");
    return -1;
  }

  // 3) Propager cet état dans la requête Motion Planning API
  ...
  /* We can also use visual_tools to wait for user input */
  visual_tools.prompt("Press 'next' in the RvizVisualToolsGui window to continue and execute trajectory");
        
  /* We execute the trajectory */
  moveit_msgs::msg::RobotTrajectory traj = response.trajectory;
    auto rc = move_group.execute(traj);
  ...
  quaternion.header.frame_id = "arm_world";
  req.path_constraints = kinematic_constraints::constructGoalConstraints("leftend_effector_link", quaternion);
